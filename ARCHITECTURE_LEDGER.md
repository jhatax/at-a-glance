...
1. Audit live source before editing.
2. Reconcile the request against this ledger and `RAID_LOG.md`.
3. State patch shape, touched files, and invariants.
4. For API, lifecycle, layout, or platform changes, review the plan before
   editing.
5. Edit narrowly.
6. Validate with `git diff --check`; run `pebble build` for code changes.
7. Use emulator screenshots for visual/layout changes.
8. Stage only intended files.
9. Do not commit API or lifecycle changes without approved before/after flow
   and commit message.
10. Update this ledger when a decision changes.

## Durable Guardrails

- Preserve user work in the dirty tree.
- Keep one coherent slice at a time.
- Do not mix architecture cleanup, visual redesign, AppMessage changes,
  platform enablement, and glyph tuning without explicit approval.
- Avoid header pollution. Public headers expose only concepts callers need.
- Keep simple calculation logic inline when that is more auditable.
- Add helpers only for real boundaries, repeated complexity, framework
  callbacks, or non-obvious domain math.
- Do not pass or return large watchface/layout structs by value.
- Do not move Pebble layer ownership out of feature modules.
- Do not introduce generic dynamic strata arrays; product strata are fixed and
  known.
- Do not mutate or copy immutable product constants into mutable profiles.
- Do not change AppMessage keys, platform support, palette, fonts, icon sizes,
  or geometry inside unrelated cleanup.
- Create paths must be transaction-like: a failed create must not leave stale
  pointers, layers, fonts, bitmaps, or partial ownership behind.
- Do not allocate heap memory in layer update procs or shared drawing
  primitives when exact static or primitive-specific storage is practical.
...
- `src/c/ataglance.h`: temporary app-level constants/debug header. Retirement
  is planned.
- `src/modules/watchface.c/.h`: runtime clearing house, live
  `WatchfaceSurface` owner, module create/destroy order, refresh dispatch, and
  source-state setter boundary.
...
- `src/modules/design.h`: current private design constants, blueprints, and
  calculated-layout structs. This is a cleanup candidate.
- `src/modules/layout_stylist.c`: palette, font-role, and custom-font
  resolution.
- `src/modules/substratum_renderer.c/.h`: shared TextLayer/Icon layer setup,
...
- `src/modules/date.c/.h`: date formatting and date text layer lifecycle.
- `src/modules/time.c/.h`: time formatting, time text layer lifecycle, and
  custom time font lifecycle.
- `src/modules/climate.c/.h`: weather source state, temperature formatting,
  weather availability, and climate text/icon lifecycle.
- `src/modules/climate_glyphs.c/.h`: Open-Meteo code mapping and procedural
  weather glyph rendering.
- `src/modules/battery.c/.h`: battery source state, track/bolt layers,
  battery color decisions, and battery refresh.
- `src/modules/bpm.c/.h`: BPM source state, BPM text/icon lifecycle, health
  reads, and BPM color decisions.
- `src/modules/steps.c/.h`: steps source state, bitmap/icon/text lifecycle,
  health reads, and steps refresh.
- `src/pkjs/index.js`: phone-side Clay bootstrap, geolocation, Open-Meteo
  fetch, request sequencing, and weather AppMessage sends.
...
- The stylist consumes `WatchfaceSurfaceStyle`; it does not rederive
  compact/full from dimensions.
- Blueprints are constants/product choices.
- Calculated layout is the resolved geometry used to lay strata on the
  surface.
- Layout-private metrics do not leak into `WatchfaceSurface`.
- Feature modules own Pebble layer lifecycle.
...
- Rectangular and round geometry may differ, but the product should read as
  the same watchface.
...
These were observed in the live tree during the June 17, 2026 audit and are
...
- `DEBUG_ATAGLANCE` should move out of product constants, preferably to a
  build define or narrow debug config header.
...
- `layout_stylist.c`, `battery.c`, `climate_glyphs.c`, and
  `substratum_renderer.c` include `ataglance.h` only for debug or inherited
  constants. These should be revisited after debug/buffer relocation.
- `src/modules/design.h` is included only by `layout_architect.c`; its structs
  and constants are implementation details and may belong in the `.c` file or
  in a more explicitly private header.
- `HELPER_CLAMP_MIN` is currently unused and its macro expression is not fully
  parenthesized.
- `HELPER_SCALE_ROUND` is used, but its expression is not fully
  parenthesized.
...
- `WatchfaceBackgroundStratum.rule_enabled` and `.rule` are currently written
  as disabled/zero and not consumed.
- `LayoutBlueprint.icon_text_pair_height` is initialized but not consumed in
...
- `src/modules/battery.c` still uses `rule` naming for the battery track layer.
  Decide whether to rename this to track vocabulary in a focused slice.
...
2. Replace `ATAGLANCE_MAX_STR_LEN` with module-local buffer constants sized to
   actual formatted values.
3. Move `DEBUG_ATAGLANCE` to a build define or narrow debug config header.
4. Remove `ataglance.h` from public module headers.
5. Remove dead `ataglance.h` includes from implementation files.
6. Delete `src/c/ataglance.h` when no symbols remain.
7. Decide whether `design.h` should collapse into `layout_architect.c` or be
...
5. Remove or reconnect unused `WatchfaceBackgroundStratum` rule fields.
6. Remove `LayoutBlueprint.icon_text_pair_height` if it remains unused.
7. Remove `DESIGN_COMPACT_RIGHT_TEXT_X`.
8. Rename battery `rule` layer names to `track` vocabulary if approved.
9. Delete tracked scratch/prototype artifacts if they are not intentional
   source artifacts.
...
3. Remove heap allocation from shared drawing paths where point counts are
   bounded.
...
3. Confirm the current round margin in `src/modules/design.h` as a product
   decision.
4. Confirm whether the unified layout needs shape-specific safe-span math
   before publication.
5. Audit all `DEBUG_ATAGLANCE` gated code before any debug geometry or message
   path can be considered safe for release.
...
4. Decide whether OAK fallback should remain fixed, be documented only, or
   become configurable.
