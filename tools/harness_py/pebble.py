...
def _load_pebble_tool() -> None:
  ...
  site_packages = next(
      (path for path in (Path(interpreter_path).parent.parent / "lib").glob("python*/site-packages")
       if path.is_dir()),
      None,
  )
...
try:
  ...
  from gen_compile_commands import main as generate_compile_database # noqa: E402
...
class PebbleAdapter:
  ...
  def set_battery(self, emulator: str, percent: int, charging: bool) -> None:
    connection = self._connection(emulator)
    try:
      send_data_to_qemu(
      ...
      self._log_failure(f"Battery {emulator}: {percent}% charging={int(charging)}", exc)
      raise
    finally:
      time.sleep(PEBBLE_SETTLE_DELAY_SECONDS)
  ...
  def screenshot(self, emulator: str, output_path: Path) -> None:
    try:
      ...
      self._log_failure(f"Screenshot {emulator}: {output_path}", exc)
      raise
    self._log_success(f"Screenshot {emulator}: {output_path}")
