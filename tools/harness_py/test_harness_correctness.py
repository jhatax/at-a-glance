...
import unittest
import json
from qaharnessconfig import QA_ROOT
...
class HarnessCorrectnessTests(unittest.TestCase):
  ...
  def test_parser_returns_parsed_scenario_without_expanding(self) -> None:
    parsed = parse_scenario(QA_ROOT / "plans" / "canary.scenario")
  ...
  def test_parser_returns_parsed_suite_with_resolved_scenarios(self) -> None:
    parsed = parse_suite(QA_ROOT / "plans" / "dev-smoke.suite")
  ...
  def test_named_scenario_loads_and_expands(self) -> None:
    plan = load_and_validate_plan("canary")
  ...
  def test_named_suite_loads_and_expands(self) -> None:
    plan = load_and_validate_plan("dev-smoke")
  ...
  def test_validate_handler_returns_process_status(self) -> None:
    self.assertEqual(handle_validate_plan("canary"), 0)

  def test_capability_arguments_are_validated_by_key(self) -> None:
    self.assertTrue(QAPlanGrammar.does_step_have_needed_attributes(
        "battery", {"charging": "0", "level": "50", "display": "white"}
    ))
    self.assertFalse(QAPlanGrammar.does_step_have_needed_attributes(
        "battery", {"charging": "0", "level": "50"}
    ))
    with self.assertRaisesRegex(ValueError, "Unsupported capability"):
      QAPlanGrammar.does_step_have_needed_attributes(
          "unknown", {"charging": "0", "level": "50", "display": "white"}
      )
  ...
  def test_report_step_accepts_capability_arguments_in_any_order(self) -> None:
    step = QAStepContext.from_dict({
        ...
        "status": "passed",
        "emulator": "emery",
        "step_args": {"charging": 0, "display": "white", "level": 50},
        "screenshot_ctx": {"expected": 0, "captured": 0, "paths": []},
    })
  ...
  def test_run_identity_matches_timestamp_and_folder(self) -> None:
    run = QARunContext.from_dict({
        "plan": "canary",
        "started_at": "20260727T154241",
        "output_folder": "/tmp/20260727T154241-pid1234",
        "_run_id": "20260727T154241-pid1234",
        "status": "passed",
        "resolved": {
        ...
        "run_outputs": {"root": "/tmp/20260727T154241-pid1234"},
        "step_count": 1,
        "step_outputs": [{
            "step_id": "battery_emery_50_0",
            "capability": "battery",
