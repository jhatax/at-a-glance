...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
  ...
  parser = argparse.ArgumentParser(
      description=("Generate compile_commands.json from a verbose Pebble SDK build log.")
  )
  parser.add_argument(
      "--log-path",
      type=Path,
  ...
  parser.add_argument(
      ...
      help="Compiler executable name or absolute path. Default: %(default)s",
  )
  parser.add_argument(
      "--verbose",
      action="store_true",
...
def resolve_compiler_path(compiler: str) -> Path:
  """Resolve a compiler token without inspecting the build log."""

  compiler_path = Path(compiler)
  if compiler_path.is_absolute():
    return compiler_path
  resolved = shutil.which(compiler)
  return Path(resolved) if resolved else compiler_path
...
def normalize_compile_arguments(args: Sequence[str], compiler_path: str,
                                project_root: Path) -> tuple[list[str], Path]:
...
def generate_compile_database(
  ...
  """Parse a verbose build log and write its compile database."""

  validate_inputs(log_path, output_path)
  platform_flag = platform_define(platform)
  project_root = log_path.resolve().parent
  compiler = str(compiler_path) if compiler_path is not None else DEFAULT_COMPILER
  ...
  if not entries_by_file:
    LOGGER.error(
        ...
        "This usually means the build log came from an incremental build "
        "with no compile steps. Run a clean verbose build and try again.",
        platform,
        log_path,
    )
  ...
  write_compile_database(sorted_entries, output_path)
  LOGGER.info(
      "Wrote %d compile commands for `%s` to %s", len(sorted_entries), platform, output_path
  )
  return EXIT_SUCCESS
...
def main(argv: Sequence[str] | None = None) -> int:
  ...
  try:
    return generate_compile_database(
        log_path=args.log_path,
        output_path=args.output,
        platform=args.platform,
        compiler_path=resolve_compiler_path(args.compiler),
    )
