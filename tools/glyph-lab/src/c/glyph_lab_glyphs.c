...
bool glyph_lab_glyphs_init(void) {
  ...
  for (int i = 0; i < STEPS_BITMAP_COUNT; ++i) {
    s_steps_bitmaps[i].bitmap = gbitmap_create_with_resource(s_steps_bitmaps[i].resource_id);
  }

  s_charge_bitmap_bolt = gbitmap_create_with_resource(RESOURCE_ID_BATTERY_BOLT_24);
...
static void draw_scaled_line_in_frame(GContext *ctx, const GRect *frame, int16_t x0, int16_t y0,
...
static void draw_unavailable_slash(GContext *ctx, const GRect *frame, GColor color) {
  graphics_context_set_stroke_color(ctx, color);
  graphics_context_set_stroke_width(ctx, 3);
  draw_scaled_line_in_frame(ctx, frame, 5, 5, 24, 24);
}

static GRect battery_scaled_rect_from_corners(const GSize *size, int16_t x0, int16_t y0, int16_t x1,
                                              int16_t y1) {
...
static void draw_scaled_filled_polygon(GContext *ctx, const GRect *frame,
                                       const GPoint *design_points, uint32_t point_count,
                                       GColor fill_color, GColor outline_color,
                                       int16_t outline_width, bool draw_outline) {
  GPoint scaled_points[8];
  GPathInfo path_info;
  GPath *path;

  if (!ctx || !frame || !design_points || point_count < 3 || point_count > 8) {
    return;
  }

  for (uint32_t i = 0; i < point_count; ++i) {
    scaled_points[i] = GPoint(
        frame->origin.x + HELPER_SCALE_ROUND(design_points[i].x, frame->size.w, DESIGN_ICON_WIDTH),
        frame->origin.y +
            HELPER_SCALE_ROUND(design_points[i].y, frame->size.h, DESIGN_ICON_HEIGHT));
  }
...
static GColor battery_color(const ColorPalette *palette, bool is_light_mode, int percent,
                            bool is_charging) {
  if (is_charging) {
  ...
  if (percent > 20) {
    return PBL_IF_COLOR_ELSE(is_light_mode ? GColorWindsorTan : GColorRajah, palette->primary_text);
  }

  return PBL_IF_COLOR_ELSE(is_light_mode ? GColorBulgarianRose : GColorRed, palette->primary_text);
}

static void draw_battery_charging_bolt(GContext *ctx, const GRect *frame,
                                       const ColorPalette *palette, GColor color) {
  glyph_lab_draw_bolt_icon(ctx, frame, palette, color);
}

static void draw_bitmap_in_frame(GContext *ctx, const GRect *frame, GBitmap *bitmap) {
  if (!ctx || !frame || !bitmap) {
    return;
  }
...
void glyph_lab_draw_bolt_icon(GContext *ctx, const GRect *frame, const ColorPalette *palette,
                              GColor color) {
  ...
  if (frame_min <= 10) {
    GPoint points[] = {
        GPoint(frame->origin.x + ((frame->size.w * 5) / 10), frame->origin.y),
        GPoint(frame->origin.x + ((frame->size.w * 2) / 10),
               frame->origin.y + ((frame->size.h * 4) / 10)),
        GPoint(frame->origin.x + ((frame->size.w * 5) / 10),
               frame->origin.y + ((frame->size.h * 4) / 10)),
        GPoint(frame->origin.x + ((frame->size.w * 3) / 10), frame->origin.y + frame->size.h - 1),
        GPoint(frame->origin.x + frame->size.w - 1, frame->origin.y + ((frame->size.h * 5) / 10)),
  ...
  // };
  static const GPoint bolt_points[] = {
      {8, 0}, {4, 14}, {8, 14}, {6, 28}, {24, 6}, {16, 6}, {20, 0},
  };

  draw_scaled_filled_polygon(ctx, frame, bolt_points, ARRAY_LENGTH(bolt_points), color,
                             palette->background, bolt_stroke_width, true);
}

void glyph_lab_draw_charge_icon(GContext *ctx, const GRect *frame, const ColorPalette *palette,
                                int variant) {
  if (!ctx || !frame || !palette) {
    return;
  }
  ...
  case CHARGE_ICON_CUSTOM_BOLT:
    glyph_lab_draw_bolt_icon(ctx, frame, palette, GColorYellow);
    return;
  case CHARGE_ICON_BITMAP_BOLT:
#ifdef PBL_BW
    helper_replace_color_in_bitmap(s_charge_bitmap_bolt, palette->background,
...
void glyph_lab_draw_battery_icon(GContext *ctx, const GRect *frame, const ColorPalette *palette,
                                 bool is_light_mode, int percent, bool is_charging) {
  if (!ctx || !frame || !palette) {
    return;
  }
  ...
  GColor draw_color = battery_color(palette, is_light_mode, percent, is_charging);
  int16_t fill_width = (percent * fill_width_max) / 100;
  GRect body = offset_rect(battery_scaled_rect_from_corners(&frame->size, shell_left, shell_top,
                                                            shell_right, shell_bottom),
                           frame->origin);
  GRect nub = offset_rect(
      battery_scaled_rect_from_corners(&frame->size, nub_left, nub_top, nub_right, nub_bottom),
      frame->origin);
  ...
  graphics_fill_rect(
      ctx,
      offset_rect(battery_scaled_rect_from_corners(&frame->size, fill_left, fill_top,
                                                   fill_left + fill_width, fill_bottom),
                  frame->origin),
      0, GCornerNone);

  if (is_charging) {
    draw_battery_charging_bolt(ctx, frame, palette, palette->primary_text);
  }
}

void glyph_lab_draw_steps_icon(GContext *ctx, const GRect *frame, const ColorPalette *palette,
                               bool is_available) {
  if (!ctx || !frame || !palette) {
    return;
  }

  GColor steps_icon_color = is_available ? palette->primary_text : palette->unavailable_text;
...
void glyph_lab_draw_steps_bitmap_icon(GContext *ctx, const GRect *frame,
                                      const ColorPalette *palette, int bitmap_kind) {
  if (!ctx || !frame || !palette) {
    return;
  }
  ...
  graphics_context_set_fill_color(ctx, palette->background);
  graphics_fill_rect(ctx, *frame, 0, GCornerNone);
  helper_replace_color_in_bitmap(bitmap, palette->background, palette->primary_text);
  graphics_context_set_compositing_mode(ctx, GCompOpSet);
  graphics_draw_bitmap_in_rect(ctx, bitmap, draw_frame);
}

static GColor bpm_color(const ColorPalette *palette, bool is_light_mode, int bpm) {
  if (bpm <= 0) {
    return palette->unavailable_text;
  }
  if (bpm > 120) {
    return PBL_IF_COLOR_ELSE(is_light_mode ? GColorBulgarianRose : GColorOrange,
                             palette->primary_text);
  }
  if (bpm >= 100) {
    return PBL_IF_COLOR_ELSE(is_light_mode ? GColorWindsorTan : GColorChromeYellow,
                             palette->primary_text);
  }
...
static void draw_bpm_icon_with_color(GContext *ctx, GColor color, const GRect *frame) {
  int16_t frame_min = HELPER_MIN(frame->size.w, frame->size.h);
  ...
  graphics_draw_rect(
      ...
      GRect(frame->origin.x + glyph_lab_scale_icon_x(&frame->size, 3),
            ...
            glyph_lab_scale_icon_x(&frame->size, 25) - glyph_lab_scale_icon_x(&frame->size, 3),
            glyph_lab_scale_icon_y(&frame->size, 23) - glyph_lab_scale_icon_y(&frame->size, 4)));
...
void glyph_lab_draw_bpm_icon(GContext *ctx, const GRect *frame, const ColorPalette *palette,
                             bool is_light_mode, int bpm, bool is_available) {
  if (!ctx || !frame || !palette) {
    return;
  }
  ...
  draw_bpm_icon_with_color(ctx, bpm_color(palette, is_light_mode, bpm), frame);
...
static int16_t weather_scale_x(const GRect *frame, int16_t value) {
  return frame->origin.x + HELPER_SCALE_ROUND(value, frame->size.w, WEATHER_ICON_WIDTH);
}

static int16_t weather_scale_y(const GRect *frame, int16_t value) {
  return frame->origin.y + HELPER_SCALE_ROUND(value, frame->size.h, WEATHER_ICON_HEIGHT);
}
...
static void weather_line(GContext *ctx, const GRect *frame, int16_t x0, int16_t y0, int16_t x1,
                         int16_t y1) {
  graphics_draw_line(ctx, weather_point(frame, x0, y0), weather_point(frame, x1, y1));
}

static void weather_fill_rect_from_corners(GContext *ctx, const GRect *frame, int16_t x0,
                                           int16_t y0, int16_t x1, int16_t y1) {
  graphics_fill_rect(ctx,
                     GRect(weather_scale_x(frame, x0), weather_scale_y(frame, y0),
                           weather_scale_x(frame, x1) - weather_scale_x(frame, x0),
                           weather_scale_y(frame, y1) - weather_scale_y(frame, y0)),
                     0, GCornerNone);
}

static void weather_fill_circle(GContext *ctx, const GRect *frame, int16_t x, int16_t y,
                                int16_t r) {
  graphics_fill_circle(ctx, weather_point(frame, x, y),
                       glyph_lab_scale_icon_coord(&frame->size, r));
}

static void weather_draw_circle(GContext *ctx, const GRect *frame, int16_t x, int16_t y,
                                int16_t r) {
  graphics_draw_circle(ctx, weather_point(frame, x, y),
                       glyph_lab_scale_icon_coord(&frame->size, r));
}
...
static GColor weather_color_for_kind(WeatherIconKind kind, const ColorPalette *palette) {
...
static void draw_weather_outline_sun(GContext *ctx, const GRect *frame, const ColorPalette *palette,
                                     bool is_outline) {
  ...
  weather_fill_circle(ctx, frame, WEATHER_ICON_CENTER_X, WEATHER_ICON_CENTER_Y, 6);
  weather_line(ctx, frame, WEATHER_ICON_CENTER_X, WEATHER_ICON_CENTER_Y - 13, WEATHER_ICON_CENTER_X,
               WEATHER_ICON_CENTER_Y - 9);
  weather_line(ctx, frame, WEATHER_ICON_CENTER_X, WEATHER_ICON_CENTER_Y + 9, WEATHER_ICON_CENTER_X,
               WEATHER_ICON_CENTER_Y + 13);
  weather_line(ctx, frame, WEATHER_ICON_CENTER_X - 13, WEATHER_ICON_CENTER_Y,
               WEATHER_ICON_CENTER_X - 9, WEATHER_ICON_CENTER_Y);
  weather_line(ctx, frame, WEATHER_ICON_CENTER_X + 9, WEATHER_ICON_CENTER_Y,
               WEATHER_ICON_CENTER_X + 13, WEATHER_ICON_CENTER_Y);
  weather_line(ctx, frame, WEATHER_ICON_CENTER_X - 10, WEATHER_ICON_CENTER_Y - 10,
               WEATHER_ICON_CENTER_X - 7, WEATHER_ICON_CENTER_Y - 7);
  weather_line(ctx, frame, WEATHER_ICON_CENTER_X + 7, WEATHER_ICON_CENTER_Y - 7,
               WEATHER_ICON_CENTER_X + 10, WEATHER_ICON_CENTER_Y - 10);
  weather_line(ctx, frame, WEATHER_ICON_CENTER_X - 10, WEATHER_ICON_CENTER_Y + 10,
               WEATHER_ICON_CENTER_X - 7, WEATHER_ICON_CENTER_Y + 7);
  weather_line(ctx, frame, WEATHER_ICON_CENTER_X + 7, WEATHER_ICON_CENTER_Y + 7,
               WEATHER_ICON_CENTER_X + 10, WEATHER_ICON_CENTER_Y + 10);
...
static void draw_weather_sun(GContext *ctx, const GRect *frame, const ColorPalette *palette) {
  draw_weather_outline_sun(ctx, frame, palette, false);
}

static void draw_weather_filled_cloud(GContext *ctx, const GRect *frame,
                                      const ColorPalette *palette, WeatherIconKind icon_kind,
                                      bool is_filled) {
  ...
  weather_draw_circle(ctx, frame, left_cloud_center_x, left_cloud_center_y, left_cloud_radius);
  weather_draw_circle(ctx, frame, center_cloud_center_x, center_cloud_center_y,
                      center_cloud_radius);
  weather_draw_circle(ctx, frame, right_cloud_center_x, right_cloud_center_y, right_cloud_radius);

  weather_fill_circle(ctx, frame, left_cloud_center_x, left_cloud_center_y, 3);
  weather_fill_circle(ctx, frame, center_cloud_center_x, center_cloud_center_y, 4);
  weather_fill_circle(ctx, frame, right_cloud_center_x, right_cloud_center_y, 3);
  weather_fill_rect_from_corners(ctx, frame, cloud_fill_left, cloud_fill_top, cloud_fill_right,
                                 cloud_fill_bottom);
  weather_line(ctx, frame, cloud_base_left, cloud_base_y, cloud_base_right, cloud_base_y);
}

static void draw_weather_cloud(GContext *ctx, const GRect *frame, const ColorPalette *palette,
...
static void draw_weather_clear_icon(GContext *ctx, const GRect *frame,
  ...
  graphics_context_set_stroke_width(ctx, 1);
  weather_draw_circle(ctx, frame, WEATHER_ICON_CENTER_X, WEATHER_ICON_CENTER_Y, 11);

  graphics_context_set_fill_color(ctx, weather_clear_fill_color(palette));
  weather_fill_circle(ctx, frame, WEATHER_ICON_CENTER_X, WEATHER_ICON_CENTER_Y, 9);
}
...
static void draw_weather_fog_icon(GContext *ctx, const GRect *frame, const ColorPalette *palette) {
  graphics_context_set_stroke_color(ctx, weather_color_for_kind(WEATHER_ICON_FOG, palette));
  graphics_context_set_stroke_width(ctx, 2);
...
static void draw_weather_drizzle_icon(GContext *ctx, const GRect *frame,
                                      const ColorPalette *palette) {
  graphics_context_set_stroke_color(ctx, weather_color_for_kind(WEATHER_ICON_DRIZZLE, palette));
  graphics_context_set_stroke_width(ctx, 2);
...
static void draw_weather_rain_icon(GContext *ctx, const GRect *frame, bool heavy,
  ...
  draw_weather_rain_marks(
      ctx, frame,
      weather_color_for_kind(heavy ? WEATHER_ICON_HEAVY_RAIN : WEATHER_ICON_RAIN, palette), heavy);
}
...
static void draw_weather_snowflake(GContext *ctx, const GRect *frame, GColor color) {
  ...
  weather_line(ctx, frame, WEATHER_ICON_CENTER_X - 7, WEATHER_ICON_CENTER_Y + 7,
               WEATHER_ICON_CENTER_X + 7, WEATHER_ICON_CENTER_Y - 7);
  weather_fill_circle(ctx, frame, WEATHER_ICON_CENTER_X, WEATHER_ICON_CENTER_Y, 2);
}
...
static void draw_weather_snow_showers_icon(GContext *ctx, const GRect *frame,
  ...
  GRect cloud_frame;
  GRect snow_frame;
  GColor color = weather_color_for_kind(WEATHER_ICON_SNOW_SHOWERS, palette);
...
static void draw_weather_bolt_icon(GContext *ctx, const GRect *frame, const ColorPalette *palette) {
  int16_t frame_min = HELPER_MIN(frame->size.w, frame->size.h);
  int16_t bolt_stroke_width = GLYPH_LAB_ICON_STROKE_WIDTH(frame_min);
  static const GPoint bolt_points[] = {
      {10, 0}, {2, 18}, {8, 18}, {5, 27}, {26, 6}, {19, 8}, {23, 0},
  };

  GColor color = weather_color_for_kind(WEATHER_ICON_THUNDERSTORM, palette);

  draw_scaled_filled_polygon(ctx, frame, bolt_points, ARRAY_LENGTH(bolt_points), color, color,
                             bolt_stroke_width, false);
}
...
void glyph_lab_draw_climate_icon(GContext *ctx, const GRect *frame, int16_t weather_condition,
  ...
  case WEATHER_ICON_SLEET:
    ...
    break;
  case WEATHER_ICON_SNOW:
    draw_weather_snowflake(ctx, frame, weather_color_for_kind(WEATHER_ICON_SNOW, palette));
    break;
  case WEATHER_ICON_SHOWERS:
