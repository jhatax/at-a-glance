...
static void draw_weather_filled_cloud(
  ...
  int stroke_width = SUBSTRATUM_RENDERER_ICON_STROKE_WIDTH(
      HELPER_MIN(frame->size.w, frame->size.h), 3);
...
static inline void draw_snowflake_segment_fixed(
    GContext* ctx,
...
static void draw_weather_snowflake(
  ...
  for (int angle_deg = 0; angle_deg < 360; angle_deg += 60) {
    ...
    draw_snowflake_segment_fixed(
      ctx, &absolute_center, cos_a, sin_a,
      0, 0, 0, -max_radius);

    // 2. Inner Chevron Branch
    draw_snowflake_segment_fixed(
      ctx, &absolute_center, cos_a, sin_a,
      0, -inner_v_pos, -feather_w, -inner_v_pos - feather_h);
    draw_snowflake_segment_fixed(
      ctx, &absolute_center, cos_a, sin_a,
      0, -inner_v_pos,  feather_w, -inner_v_pos - feather_h);
    ...
    draw_snowflake_segment_fixed(
      ctx, &absolute_center, cos_a, sin_a,
      0, -outer_v_pos, -feather_w, -outer_v_pos - feather_h);
    draw_snowflake_segment_fixed(
      ctx, &absolute_center, cos_a, sin_a,
      0, -outer_v_pos,  feather_w, -outer_v_pos - feather_h);
    ...
    draw_snowflake_segment_fixed(
      ctx, &absolute_center, cos_a, sin_a,
      0, -max_radius, -1, -max_radius - 1);
    draw_snowflake_segment_fixed(
      ctx, &absolute_center, cos_a, sin_a,
      0, -max_radius,  1, -max_radius - 1);
    draw_snowflake_segment_fixed(
      ctx, &absolute_center, cos_a, sin_a,
      -1, -max_radius - 1, 0, -max_radius - 2);
    draw_snowflake_segment_fixed(
      ctx, &absolute_center, cos_a, sin_a,
      1, -max_radius - 1, 0, -max_radius - 2);
  }
