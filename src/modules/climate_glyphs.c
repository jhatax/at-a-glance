...
static void draw_snowflake_segment_fixed(
  ...
  GPoint scaled_1 = GPoint(center->x + HELPER_SCALE_ROUND(r_x1, 1, TRIG_MAX_RATIO),
    center->y + HELPER_SCALE_ROUND(r_y1, 1, TRIG_MAX_RATIO));
  GPoint scaled_2 = GPoint(center->x + HELPER_SCALE_ROUND(r_x2, 1, TRIG_MAX_RATIO),
...
static void draw_weather_unavailable_icon(
  ...
  substratum_renderer_draw_unavailable_slash(ctx, &frame->size, climate_palette->normal);
}
