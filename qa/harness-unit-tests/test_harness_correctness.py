...
class HarnessCorrectnessTests(unittest.TestCase):
    ...
    def test_direct_scenario_path_loads(self) -> None:
        path = QA_ROOT / "scenarios" / "canary.scenario"
    ...
    def test_direct_suite_path_loads(self) -> None:
        path = QA_ROOT / "scenarios" / "dev-smoke.suite"
        plan = validate_plan(str(path))
        self.assertEqual(plan.name, "dev-smoke")
        self.assertEqual(plan.path, path)
    ...
    def test_double_tap_scenario_loads(self) -> None:
        plan = validate_plan("emery-double-tap")
        step = next(iter(plan.steps.values()))
        self.assertEqual(step.capability, "double-tap")
        self.assertEqual(step.fields["display"], "black")
